...
const filterRecipes = () => {
  ...
  filteredRecipes = filteredRecipes.filter(recipe => {
    ...
    const hasUstensils = selectedUstensils.every(ustensil =>
      recipe.ustensils.some(item => item.toLowerCase() === ustensil.toLowerCase())
    );
...
const handleChevronToggle = (showChevron, hideChevron, listContainer, input, items) => {
    ...
    searchInput.classList.add('mini-search');
  
    const searchIcon = document.createElement('i');
    searchIcon.classList.add('fa', 'fa-search', 'mini-search-icon');
  
    const searchWrapper = document.createElement('div');
    searchWrapper.classList.add('mini-search-wrapper');
    ...
    showChevron.addEventListener("click", () => {
      ...
      listContainer.insertBefore(searchWrapper, listContainer.firstChild);
      searchInput.focus();
    });
